...
- New batches persist their selection, language, source identity, and per-chapter
  ...
  and failed chapters, and provides its own retry action. Retry uses ascending
  chapter-index order and checkpoint state, even if an unfinished chapter has
  older cached text; opening the screen never starts Speech.
...
- Support Bold Text ([GitHub issue #44](https://github.com/terminaloutcomes/bleat/issues/44)),
  ...
  and Reduce Motion (completed maintainer audit recorded in
