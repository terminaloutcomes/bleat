...
@Suite(.serialized)
final class TokenVaultTests {
    @Test(
        .enabled(
            if: keychainHostAvailable, "Requires a host Keychain entitlement"))
        ...
        async throws
    {
        ...
        try await withTestCleanup(
            ...
            operation: {
                ...
                let initiallyStored = try await store.credentials(
                    for: firstAccount)
                #expect(initiallyStored == nil)
                ...
                let loadedSecond = try await store.credentials(
                    for: secondAccount)
                let loadedFirstNativeLogin =
                    try await store.nativeLoginCredentials(for: firstAccount)
                #expect(loadedFirst == firstTokens)
                #expect(loadedSecond == secondTokens)
                #expect(loadedFirstNativeLogin == firstNativeLogin)
                ...
                #expect(loadedReplacement == replacementTokens)
                #expect(retainedNativeLogin == firstNativeLogin)
                #expect(hasExpectedAccessibility)

                try await store.deleteCredentials(for: firstAccount)
                try await store.deleteCredentials(for: firstAccount)
                let deletedFirst = try await store.credentials(
                    for: firstAccount)
                let deletedNativeLogin =
                    try await store.nativeLoginCredentials(for: firstAccount)
                let retainedSecond = try await store.credentials(
                    for: secondAccount)
                #expect(deletedFirst == nil)
                #expect(deletedNativeLogin == nil)
                #expect(retainedSecond == secondTokens)
            })
    }
        ...
        async throws
    {
        ...
        try await withTestCleanup(
            ...
            operation: {
                ...
                try await store.save(
                    tokens, nativeLogin: login, for: firstAccount)
                try await store.save(
                    tokens, nativeLogin: login, for: secondAccount)

                try await store.deleteAllCredentials()

                let firstTokens = try await store.credentials(for: firstAccount)
                let secondTokens = try await store.credentials(
                    for: secondAccount)
                ...
                #expect(firstTokens == nil)
                #expect(secondTokens == nil)
                #expect(firstLogin == nil)
                #expect(secondLogin == nil)
            })
    }
        ...
        async throws
    {
        ...
        try await withTestCleanup(
            ...
            operation: {
                ...
                let migratedTokens = try await store.credentials(
                    for: canonicalID)
                let migratedLogin = try await store.nativeLoginCredentials(
                    for: canonicalID
                )
                #expect(migratedTokens == canonicalTokens)
                #expect(migratedLogin == canonicalLogin)
            })
    }
    ...
    func testRejectsEmptyAccountID() async throws {
        ...
        await assertThrowsErrorAsync(
            ...
            #expect(error as? TokenVaultError == .invalidAccountID)
        }
        await assertThrowsErrorAsync(
            try await store.save(
    ...
    func testRejectsEmptyService() async throws {
        ...
        await assertThrowsErrorAsync(
            ...
            #expect(error as? TokenVaultError == .invalidService)
        }
    }
    ...
    func testRejectsMalformedStoredCredentials() async throws {
        ...
        await assertThrowsErrorAsync(
            ...
            #expect(error as? TokenVaultError == .invalidStoredCredentials)
        }
    }
    ...
    func testMissingEntitlementStatusHasTypedError() {
        if let error = #expect(
            ...
            performing: { try TokenVault.check(errSecMissingEntitlement) })
        {
            #expect(error as? TokenVaultError == .missingEntitlement)
        }
    }
