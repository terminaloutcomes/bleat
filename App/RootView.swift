                ...
                List {
                    ...
                    Section {
                        LabeledContent(
                            "Time Listening",
                            value: duration(
                                summary.realSeconds + liveRealSeconds)
                        )
                        .accessibilityLabel(
                            "Time listening, \(range.title), \(scopeLabel), \(coverageLabel(summary))"
                        )
                        if summary.realTimeCoverage != .thisApp {
                            LabeledContent(
                                "This App",
                                value: duration(
                                    summary.localRealSeconds
                                        + liveRealSeconds)
                            )
                        }
                        LabeledContent(
                    ...
                    if case .loaded(let exploration) = model
                    ...
                    Section("Your Data") {
                        if let account = selectedAccount {
                            ...
                            Button("Export JSON") {
                                Task {
                                    if let data = await model.exportStatistics(
                                        ...
                                        exportDocument = StatisticsJSONDocument(
                                            data: data)
                                        exporting = true
                                    }
                                }
                            ...
                            Button(
                                "Reset Listening Statistics", role: .destructive
                            ) {
...
private struct RemoteTelemetryConsentSection: View {
    ...
    var body: some View {
        Section {
            ...
            Text(
                "Shares bounded technical diagnostics such as app and operating-system versions, operation outcomes, timing, and a random installation identifier used to correlate related events. Audiobook content, credentials, accounts, servers, searches, transcripts, and hardware or advertising identifiers are excluded. You can turn this off at any time without affecting local Diagnostics."
            )
